...
def create_meeting_schedule(select_files, output_file):
    ...
    sf_= pd.read_excel(select_files())

    # Tüm DataFrame'leri birleştir ve ortak isimleri bul
    # combined_df = pd.concat([df1, df2, df3])
    combined_df = pd.concat([sf_,sf_])
    # common_names = combined_df['İsim'].value_counts()[combined_df['İsim'].value_counts() > 1].index.tolist()
    common_names = combined_df['Numara'].value_counts()[combined_df['Numara'].value_counts() > 1].index.tolist()

    print("çıkan ekrana başlangıç tarihi ve saati giriniz")
...
create_meeting_schedule(select_files(), output_file)
